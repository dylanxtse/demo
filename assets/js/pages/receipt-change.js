(function () {
  window.RecordPageConfig = {
    ...
    rowActions: [
      { key: 'approve', label: '审核', href: (item) => `./receipt-change-form.html?mode=audit&id=${encodeURIComponent(item.id)}`, visible: ['PENDING'] },
      { key: 'edit', label: '编辑', visible: ['PENDING'] },
      { key: 'close', label: '关闭', transition: 'close', visible: ['PENDING', 'APPROVED'], confirmTitle: '关闭实收变更', message: '确定要关闭该订单吗？' },
      { key: 'delete', label: '删除', danger: true, visible: ['PENDING'] }
    ],
    formFields: [
    ...
    createDefaults: { status: 'PENDING', creator: '当前用户', auditAt: '', auditor: '' },
