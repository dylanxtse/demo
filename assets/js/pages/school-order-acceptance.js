(function () {
  ...
  const display = (line) => line.displayName || `${line.productName || line.goodsName || '--'}(${line.unit || '--'}/${line.brand || '--'}/${line.spec || '--'})`;
  const state = {
  ...
  function render() {
    const content = `<section class="school-order-acceptance-page" id="schoolOrderAcceptancePage" aria-label="订单验收">
      <header class="school-order-acceptance-header"><button type="button" class="school-order-acceptance-back" data-action="back" aria-label="返回订单管理">‹ <span>返回</span></button><h1>订单验收</h1></header>
