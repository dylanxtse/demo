(function () {
  ...
  var allocationSaveAnnotation = {
    id: 'purchase-task-allocation-enterprise-date',
    target: 'custom',
    targetSelector: '[data-action="save"]',
    placement: 'left',
    title: '生成采购单时提交企业期望送达时间',
    items: ['保存分配并生成采购单时，企业期望送达时间默认提前2天，且不能晚于学校期望送达时间。']
  };
  ...
  var content = [
    '<section class="page-card purchase-page purchase-detail-page purchase-task-allocation-page" aria-label="采购任务分配">',
      ...
      '<div class="purchase-detail-footer"><span class="record-annotation-entry"><button class="btn btn-primary" type="button" data-action="save">保存</button>' + renderAnnotationMarker(allocationSaveAnnotation, 'save', true) + '</span></div>',
    '</section>'
  ].join('');
  ...
  window.AnnotationOverlay?.mount?.(annotationRoot, new Map([[allocationSaveAnnotation.id, allocationSaveAnnotation]]));
