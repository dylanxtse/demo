(function () {
  ...
  var schoolDateAnnotation = {
    id: 'purchase-order-form-school-expected-date',
    target: 'custom',
    targetSelector: '#purchaseExpectedAt',
    placement: 'right',
    title: '学校期望送达时间',
    items: ['原“期望送达时间”调整为“学校期望送达时间”。']
  };
  var enterpriseDateAnnotation = {
    id: 'purchase-order-form-enterprise-expected-date',
    target: 'custom',
    targetSelector: '#purchaseEnterpriseExpectedAt',
    placement: 'right',
    title: '企业期望送达时间',
    items: ['采购任务生成采购单时提交的企业期望送达时间，在编辑采购单时只读。']
  };
  var batchAddAnnotation = {
    id: 'purchase-order-form-hide-batch-add',
    target: 'custom',
    targetSelector: '.purchase-form-table',
    placement: 'left',
    title: '隐藏批量添加商品',
    items: ['采购单编辑时隐藏“批量添加商品”功能。']
  };
    ...
    '<section class="page-card purchase-page purchase-form-page" aria-label="' + title + '">',
      ...
      '<div class="purchase-form-scroll">',
        '<div class="purchase-basic-form">',
          ...
          '<div class="purchase-basic-field"><label class="field-label required" for="purchaseEnterpriseExpectedAt">企业期望送达时间</label>' + renderAnnotationMarker(enterpriseDateAnnotation) + '<input class="form-control" id="purchaseEnterpriseExpectedAt" type="text" placeholder="请选择日期" readonly value="' + text(enterpriseAt) + '"></div>',
          '<div class="purchase-basic-field"><label class="field-label required" for="purchaseExpectedAt">学校期望送达时间</label>' + renderAnnotationMarker(schoolDateAnnotation) + '<input class="form-control" id="purchaseExpectedAt" type="text" placeholder="请选择日期" readonly' + (readonlyBase ? ' disabled' : '') + ' value="' + text(expectedAt) + '"></div>',
        ...
        '<section class="purchase-form-section">',
          '<div class="purchase-section-heading">' + (mode === 'edit' ? renderAnnotationMarker(batchAddAnnotation) : '<button class="btn btn-primary btn-sm" type="button" data-action="batch-add">批量添加商品</button>' + renderAnnotationMarker(batchAddAnnotation, 'batch-add', true)) + '</div>',
  ...
  var annotationRoot = root.querySelector('#pageContent') || root;
  window.AnnotationOverlay?.mount?.(annotationRoot, new Map([
    [schoolDateAnnotation.id, schoolDateAnnotation],
    [enterpriseDateAnnotation.id, enterpriseDateAnnotation],
    [batchAddAnnotation.id, batchAddAnnotation]
  ]));
