(function () {
  ...
  var schoolDateAnnotation = {
    id: 'purchase-order-school-expected-date',
    target: 'custom',
    targetSelector: '#orderDeliveryDisplay',
    placement: 'right',
    title: '学校期望送达时间',
    items: ['采购单列表支持按学校期望送达时间查询和展示。']
  };
  var enterpriseDateAnnotation = {
    id: 'purchase-order-enterprise-expected-date',
    target: 'custom',
    targetSelector: '#orderEnterpriseDeliveryDisplay',
    placement: 'right',
    title: '企业期望送达时间',
    items: ['采购单列表新增企业期望送达时间查询和展示，时间在生成采购单时提交。']
  };
  var manualAddAnnotation = {
    id: 'purchase-order-hide-manual-add',
    target: 'custom',
    targetSelector: '.purchase-order-toolbar',
    placement: 'left',
    title: '隐藏手动添加采购单',
    items: ['采购单列表隐藏手动添加采购单功能，采购单由采购任务生成。']
  };
  service.ensureSeed();
  ...
  window.AnnotationOverlay?.mount?.(annotationRoot, new Map([
    ...
    [manualAddAnnotation.id, manualAddAnnotation]
  ]));
  var state = {
    page: 1,
    pageSize: 20,
