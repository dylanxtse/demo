(function () {
  ...
  var taskDateAnnotation = {
    id: 'purchase-task-school-expected-date',
    target: 'custom',
    targetSelector: '#taskDate',
    placement: 'right',
    title: '学校期望送达时间',
    items: ['原“期望发货日期”调整为“学校期望送达时间”，用于承接学校侧的送达要求。']
  };
  var generateOrderAnnotation = {
    id: 'purchase-task-generate-order',
    target: 'custom',
    targetSelector: '[data-action="generate-order"]',
    placement: 'left',
    title: '生成采购单',
    items: ['生成采购单时补录“企业期望送达时间”；默认比学校期望送达时间提前2天，且不能晚于学校期望送达时间。']
  };
  ...
  var content = [
    '<section class="page-card purchase-page purchase-list-page purchase-task-page" aria-label="采购任务">',
      ...
      '<div class="purchase-toolbar">',
        '<div class="purchase-toolbar-main"><button class="btn btn-primary" type="button" data-action="batch-allocate">批量分配</button><span class="record-annotation-entry"><button class="btn btn-primary" type="button" data-action="generate-order">生成采购单</button>' + renderAnnotationMarker(generateOrderAnnotation, 'generate-order', true) + '</span></div>',
  ...
  window.AnnotationOverlay?.mount?.(annotationRoot, new Map([
    [taskDateAnnotation.id, taskDateAnnotation],
    [generateOrderAnnotation.id, generateOrderAnnotation]
  ]));
