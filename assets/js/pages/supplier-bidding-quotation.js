  ...
  function renderRows(root, state) {
    ...
    body.innerHTML = visible.length
      ? visible.map((row, index) => `<tr>
          <td>${start + index + 1}</td>
          <td><a class="supplier-bid-number" href="./supplier-bid-detail.html?id=${encodeURIComponent(row.id)}">${escapeHtml(row.bidNo)}</a></td>
  ...
  function render() {
    ...
    page.addEventListener('click', (event) => {
      ...
      if (action === 'quote') {
        ...
        if (row?.canQuote) navigate(`./supplier-bidding-quotation-form.html?id=${encodeURIComponent(row.id)}`);
        return;
      }
