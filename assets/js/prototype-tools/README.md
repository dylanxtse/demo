...
标注和迭代记录面板的可复用工具包。工具层独立挂载到页面覆盖层，不参与业务组件布局；主题默认读取宿主项目的 `--primary`，生成对比色作为辅助工具强调色。
...
默认主题流程是：读取宿主项目 `--primary` → 取互补色 `H + 180°` → 保持 `H` 不变并将 `S/L` 校正到 `55%～75%` → 将校正后的颜色应用到标注和迭代面板。若项目需要指定辅助色，可以传入同样的 `apply` 接口：
